...
totalNumChar=$(cat $INPUT | tr -d '\n' | wc -c)
totalPrintedChar=$(sed -e 's/\\\\/|/g' -e 's/\\\"/|/g' -e 's/\\x../|/g' $INPUT | tr -d '\"\n' | wc -c)
totalReencodedChar=$(sed -e 's/"/@/g' -e 's/\\/#/g' -e 's/@/\\"/g' -e 's/#/\\\\/g' -e 's/\(.*\)/\"\1\"/g' input | tr -d '\n' | wc -c)

echo Part 1: $(($totalNumChar - $totalPrintedChar))
echo Part 2: $(($totalReencodedChar - $totalNumChar))
