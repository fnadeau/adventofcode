...
echo captcha answer 1 is $sum

captcha=$(<$INPUT)

captchalenhalf=$((${#captcha} / 2))

captcha1+=("${captcha:0:$captchalenhalf}")
captcha2+=("${captcha:$captchalenhalf}")

sum=0
for i in $(seq 0 $(($captchalenhalf - 1)))
do
    if [ "${captcha1:$i:1}" == "${captcha2:$i:1}" ]; then
        ((sum += ${captcha1:$i:1}))
    fi
done
((sum*=2))
echo captcha answer 2 is $sum
