...
declare -a reindeerName

if [ ! -f "${INPUT}" ]
then
	echo Usage: $0 input_file time
	exit 1
fi
...
do
	...
	reindeerName+=("${array[0]}")
	declare ${array[0]}_speed=${array[3]}
	declare ${array[0]}_s1=${array[6]}
	declare ${array[0]}_c1=${array[6]}
	declare ${array[0]}_s2=${array[13]}
	declare ${array[0]}_c2=${array[13]}
	declare ${array[0]}_dist=0
	declare ${array[0]}_score=0
done < "${INPUT}"

for i in $(seq 1 $TIME)
do
	# process distance and sleep
	for reindeer in "${reindeerName[@]}"
	do
		c1="${reindeer}_c1"
		c2="${reindeer}_c2"
		if [ "${!c1}" -gt "0" ]
		then
			speed="${reindeer}_speed"
			dist="${reindeer}_dist"
			tmp=$((${!c1} - 1))
			eval "${c1}=\$tmp"
			tmp=$((${!dist} + ${!speed}))
...
do
	dist="${reindeer}_dist"
	score="${reindeer}_score"
	echo -e "$reindeer scored\t${!score}\twith total distance of\t${!dist}"
done
