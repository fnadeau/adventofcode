...
if [[ $(cat $INPUT) =~ .*row[[:space:]]([0-9]*),[[:space:]]column[[:space:]]([0-9]*).$ ]]
then
	seekRow=${BASH_REMATCH[1]}
	seekColumn=${BASH_REMATCH[2]}
	echo Calculating code at row $seekRow, column $seekColumn
else
	echo Invalid input
	exit 1
fi
...
do
	...
	if [[ "$column" -eq "$seekColumn" && "$row" -eq "$seekRow" ]]
	then
		echo code is $lastValue
		break
	fi
